...
try:
    yolo_model = YOLO("yolov8n.pt") # Using nano for faster webcam testing
except Exception as e:
...
def capture_camera():
    ...
    # Optional: Force a standard resolution to prevent scaling bugs
...
def process_ai():
    ...
    while True:
        ...
        # 2. STRICT FACE RECOGNITION (No YOLO math offsets!)
        if human_in_frame:
            # Scale down to 50% (0.5) instead of 25%. 25% is too blurry for accurate IDs when sitting back.
            small_frame = cv2.resize(frame_to_analyze, (0, 0), fx=0.5, fy=0.5)
            rgb_small_frame = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB)
            
            # Use dlib's native, highly accurate bounding boxes
            face_locations = face_recognition.face_locations(rgb_small_frame)
            face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations)
            
            for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                # Dropped tolerance to 0.48 to prevent Vanshika/Aditya false matches
                ...
                # Scale the bounding box back up by 2 (since we scaled down by 0.5)
                ...
                now = time.time()
                cooldown_key = name
                if now - LAST_LOG_TIME.get(cooldown_key, 0) > 5.0: # 5 second cooldown
                    ...
                    if len(DETECTIONS) > 50:
                        DETECTIONS.pop()
                    LAST_LOG_TIME[cooldown_key] = now
...
def generate_frames():
    ...
    while True:
        ...
        # Draw standard Objects (Blue Boxes)
